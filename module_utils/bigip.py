...
class tmos:
    ...
    # get FUNCTION
    #-----------------------------------------------------------------------
    def get_legacy(self, uri, headers = None, format = 'json'):
    ...
    # post FUNCTION
    #-----------------------------------------------------------------------
    def post_legacy(self, uri, data, headers = None, format = 'json'):
        if format == 'json':
            if headers:
    ...
    # patch FUNCTION
    #-----------------------------------------------------------------------
    def patch_legacy(self, uri, data, headers = None, format = 'json'):
        if format == 'json':
            if headers:
                headers['Content-Type'] = 'application/json'
            else:
    ...
    #-----------------------------------------------------------------------
    # install_custom_monitor FUNCTION
    #-----------------------------------------------------------------------

    def install_custom_monitor (self, file):
        filename = os.path.basename(file)
        monitor_name = filename.split('.')[0]
        postData = {"name": monitor_name ,"partition":"Common","source-path":"file:/var/config/rest/downloads/" + filename}
        res = self.post('/mgmt/tm/sys/file/external-monitor/', data = postData)
        if res.status_code >= 400:
            return ""
        else:
            d = res.json()
    ...
    def iapplx_install_package(self, file):
        filename = os.path.basename(file)
        postData = {"operation":"INSTALL","packageFilePath":"/var/config/rest/downloads/"+ filename}
        res = self.post('/mgmt/shared/iapp/package-management-tasks', data = postData)
        if res.status_code >= 400:
            return ""
        else:
            d = res.json()
    ...
    def iapplx_check_task_status(self, task_id):
        res = self.get( '/mgmt/shared/iapp/package-management-tasks/'+ task_id )
